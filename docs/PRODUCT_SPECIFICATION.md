...
字幕焼き込みは保留機能とする。SRT利用実績と要望が確認できた場合だけ、libass、Windowsパス、日本語フォント、再エンコード時間を技術spikeで評価する。SRTと同じ初期フェーズへ含めない。
...
- WebUIは動画登録とは独立した「LLM要約・見どころ」タブを持ち、その内側を「① 要約を作る・確認する」「② 要約から見どころを作る・切り抜く」の2タブに分離する。対象動画の選択欄は各作業タブ内に置き、どちらから変更しても他方へ同期する。各タブは元動画の再生previewと、表示名・長さ・文字起こし／要約状態を表示する。動画選択はactive Transcript revisionにreadyな解析結果がある動画を「要約済み」と明示する。選択時に保存済み要約と候補状態を読み込み、ready解析があれば再要約せず候補生成を有効化し、なければ誤操作できないよう無効化する。
...
- 生成候補の選択Radioは別の選択欄へ分離せず、各候補の番号・時間・タイトルの直前に配置する。説明を含む候補行全体をlabelとしてクリック可能にし、選択状態と内容を同じ場所で確認できるようにする。保存は利用者の明示操作後に「選択候補のみ」または「表示中の全候補」を個別mp4へ切り抜く。保存名は `元動画名_章タイトル.mp4` を基本とし、Windows禁止文字・予約名を無害化する。同名時は連番を付ける。途中ファイルから最終名への置換前にclaimを取得し、既存ファイルを上書きせず、失敗時はpartialとclaimを除去する。
