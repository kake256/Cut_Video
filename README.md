...
1. **「動画保存」タブ**でURLまたは動画ファイルを選び「インデックス作成」(動画の長さに応じて数分)
2. **「検索・編集・切り抜き」タブ**の「検索対象の動画を選ぶ」を開き、サムネイルカードから対象を選んでクエリを検索（ファイル名で絞り込み可能）
...
切り抜き動画、ダウンロード動画、共有用インデックスzipは、それぞれの画面にある
...
WebUIでは独立した「LLM要約・見どころ」内を「① 要約を作る・確認する」と
「② 要約から見どころを作る・切り抜く」の2タブに分けています。`[要約済み]` の動画を
選ぶと保存済みの要約・時間付き章・候補状態を読み込み、再要約せず候補を生成できます。
対象動画は各タブ内のサムネイル一覧または代替Dropdownから選択でき、両方の選択状態は同期します。
サムネイルカードには動画名・長さ・文字起こし／索引／要約状態を表示し、未要約カードは
選択可能なまま減光して、処理前に対象を見分けられるようにしています。
候補は個別preview、編集画面への読み込み、選択候補または全候補の明示的な切り抜き保存に
対応します。映像だけの出来事、表情、音の盛り上がりは評価対象外です。
...
「表示中の全候補」を明示的な保存操作でローカル動画として切り抜けます。保存名は
`元動画名_章タイトル.mp4` を基本とし、Windowsで使えない文字を置換します。既存ファイルは上書きしません。

候補の最大尺は既定180秒です。短い候補を一律に引き延ばす値ではなく、導入・本題・着地に
追加の発話が必要なときにだけ使える上限です。UIまたはCLIで動画に合わせて変更できます。

既にreadyなLLM解析がある動画ではCLIからも生成できます。

```powershell
venv\Scripts\python.exe generate_highlights.py --video-id <公開動画ID> --count 6 --min-duration 20 --max-duration 180
```
