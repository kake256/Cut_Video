...
- WebUIは動画登録とは独立した「LLM要約・見どころ」タブを持ち、その内側を「① 要約を作る・確認する」「② 要約から見どころを作る・切り抜く」の2タブに分離する。両タブは同じ選択動画を引き継ぐ。動画選択はactive Transcript revisionにreadyな解析結果がある動画を「要約済み」と明示する。選択時に保存済み要約と候補状態を読み込み、ready解析があれば再要約せず候補生成を有効化し、なければ誤操作できないよう無効化する。
